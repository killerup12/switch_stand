...
> Файлы передавать через SMB или записывать через `/file/set contents=`.
> veth, bridge ports и mounts **переживают** роллбек RouterOS-бэкапа — проверь `print` перед созданием.

---

## Шаг 1 — Скопировать файлы на роутер

SCP upload не работает. Используй SMB (USB-шара включена по умолчанию):
...
# Скопировать папку Documents/vpn-ui/etc/ → usb1/docker/vpn-ui/etc/
```
